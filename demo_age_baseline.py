...
age_detection_model_path = './model/age_baseline.hdf5'


def load_model(face_cascade_model_path, age_baseline_model_path):
    face_cascade = cv2.CascadeClassifier(face_cascade_model_path)
    age_baseline_image_shape0 = 64
    age_baseline_model = age_baseline_net((age_baseline_image_shape0, age_baseline_image_shape0, 3))
    age_baseline_model.load_weights(age_baseline_model_path)
    return face_cascade, age_baseline_model, age_baseline_image_shape0
...
def main():
    face_cascade, age_baseline_model, age_model_shape = load_model(face_detection_model_path, age_detection_model_path)
    ...
    while True:
        ...
        if len(face_rois) > 0:
            # tic = time.process_time()
            predicted_ages = age_baseline_model.predict(x_test)
            # toc = time.process_time()
